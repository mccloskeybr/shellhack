...
Win32_FormatEnginePixelBuffer(
  ...
  // NOTE: the game buffer stores data flipped in the y direction
  // compared to windows, so start at the end and decrement rows until
  // we've reached the beginning of the buffer.
  Pixel* game_data_row = pixel_buffer->buffer +
    (pixel_buffer->width * pixel_buffer->height) -
    pixel_buffer->width;
  uint8_t* os_data_row = (uint8_t*) formatted_pixel_data;
  for (int y = 0; y < pixel_buffer->height; ++y) {
    Pixel* game_pixel = game_data_row;
    uint32_t* os_pixel = (uint32_t*) os_data_row;

    for (int x = 0; x < pixel_buffer->width; ++x) {
      *os_pixel =
        (uint32_t) game_pixel->color.r << 16 |
        (uint32_t) game_pixel->color.g << 8  |
        (uint32_t) game_pixel->color.b;

      game_pixel++;
      os_pixel++;
    }

    game_data_row -= pixel_buffer->width;
    os_data_row += pixel_buffer->width * BYTES_PER_PIXEL;
  }
}
