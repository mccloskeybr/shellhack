...
Win32_InitDirectSound(
	...
	OutputDebugString("Direct Sound initialized\n");
	return OK;
}
...
Win32_FormatPrimarySoundBuffer(
	...
	OutputDebugString("Primary sound buffer set\n");
	return OK;
}
...
Win32_InitSecondarySoundBuffer(
	...
	OutputDebugString("Secondary sound buffer set\n");
	return OK;
}
...
Win32_ClearSoundBuffer(LPDIRECTSOUNDBUFFER* buffer) {
	...
	if(!SUCCEEDED(result)) {
		// log
		OutputDebugString("ERROR clearing secondary buffer\n");
		return INTERNAL;
	}
	uint8_t* sample;
	...
	OutputDebugString("secondary buffer cleared\n");
	return OK;
}
...
Win32_TryFillSoundBuffer(
	...
	if (sample_buffer_byte_size >= sound_buffer_byte_size) {
		OutputDebugString("Provided sample buffer exceeds configured limit!\n");
	...
	DWORD play_cursor_unsigned;
		...
		OutputDebugString("Unable to get cursor positions!\n");
		return INTERNAL;
	}
	...
	if (play_cursor < start_cursor && start_cursor < write_cursor) {
		...
		OutputDebugString("Start cursor in-between play and write cursor!\n");
		return FAILED_PRECONDITION;
	}
	if (play_cursor < end_cursor && end_cursor < write_cursor) {
		OutputDebugString("End cursor in-between play and write cursor!\n");
		return FAILED_PRECONDITION;
	}
	if (start_cursor < play_cursor && write_cursor < end_cursor) {
		OutputDebugString("Requested sample buffer would overflow!\n");
		return FAILED_PRECONDITION;
	}
		...
		OutputDebugString("Could not retrieve lock!\n");
		return INTERNAL;
	}
}
...
Platform_CreateSoundDevice(
	...
	*device = (Platform_SoundDevice*) malloc(sizeof(Platform_SoundDevice));
	...
	RETURN_IF_ERROR(Win32_ClearSoundBuffer(&((*device)->sound_buffer)));
 	OutputDebugStringA("direct sound loaded successfully\n");
	return OK;
}
